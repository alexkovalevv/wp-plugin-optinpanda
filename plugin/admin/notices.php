<?php
	...
	function onp_op_admin_premium_notices($notices)
	{
		...
		if( (!$optinpanda->license || $optinpanda->license->build !== 'free' || $optinpanda->build !== "free") && !$forceToShowNotices ) {
			return $notices;
		}
		...
		// shows every 10 days
		if( ($lastCloase + DAY_IN_SECONDS * 7 > time()) || $forceToShowNotices ) {
				...
				$notices[] = array(
					...
					'buttons' => array(
						array(
							...
							'action' => onp_licensing_000_manager_link($optinpanda->pluginName, 'activateTrial', false)
						),
						array(
				...
				$notices[] = array(
					...
					'buttons' => array(
						array(
							...
							'class' => 'button button-primary',
							'action' => onp_licensing_000_get_purchase_url($optinpanda)
						),
						array(
	...
	class OnpOP_RateUs_Popup {
		...
		public function getAchievementAction()
		{
			...
			if( $action && isset($_COOKIE['onp_op_' . $action . '_closed']) ) {
				return false;
			}
		...
		public function getData()
		{
			...
			$data = array(
				'id' => $this->getId(),
				'class' => 'onp-op-rateus-popup factory-fontawesome-000',
				'position' => 'popup',
				'layout' => 'custom',
				'close' => 'quick-hide',
