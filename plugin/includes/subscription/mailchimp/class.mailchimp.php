...
class OPanda_MailchimpSubscriptionService extends OPanda_Subscription {
    ...
    public function getGroups( $listId ) {
        ...
        $response = $MailChimp->get("lists/$listId/interest-categories");
    ...
    public function subscribe( $identityData, $listId, $doubleOptin, $contextData, $verified ) {
        ...
        $MailChimp = $this->initMailChimpLibs(3);
        $response = $MailChimp->post("lists/$listId/members", array(
            'email_address'     => $email,
            'status'            => ( $verified ? true : !$doubleOptin ) ? 'subscribed' : 'pending',
            'merge_fields'        => $vars,
            'interests'         => $interests,
            'double_optin'      => $verified ? false : $doubleOptin,
            'send_welcome'      => $verified ? $sendWelcomeMessage : (!$doubleOptin ? $sendWelcomeMessage : false)
        ));

        if( isset($response['title']) && strpos( 'Member Exists', $response['title'] ) === false ) {
            throw new OPanda_SubscriptionException ( '[subscribe]: ' . $response['title'] );
            
        // already exits
            
        }  else if ( isset($response['title']) && strpos( 'Member Exists', $response['title'] ) !== false ) {

            $hash = md5( strtolower($email) );
            $response = $MailChimp->put("lists/$listId/members/$hash", array(
                'email_address'     => $email,
                'merge_fields'      => $vars,
                'interests'         => $interests,
            ));
