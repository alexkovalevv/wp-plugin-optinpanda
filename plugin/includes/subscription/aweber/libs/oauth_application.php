<?php
if (!class_exists('OPanda_CurlObject')) require_once('curl_object.php');
if (!class_exists('OPanda_CurlResponse')) require_once('curl_response.php');
 ...
 * @version $id$
 */
interface OPanda_OAuthServiceProvider {
 ...
 * @version $id$
 */
class OPanda_OAuthApplication implements OPanda_AWeberOAuthAdapter {
    public $debug = false;
    ...
    /**
     * @var OPanda_OAuthUser User currently interacting with the service provider
     */
    public $user = false;
    ...
    public function __construct($parentApp = false) {
        if ($parentApp) {
            if (!is_a($parentApp, 'OPanda_OAuthServiceProvider')) {
                throw new Exception('Parent App must be a valid OAuthServiceProvider!');
            }
            $this->app = $parentApp;
        }
        $this->user = new OPanda_OAuthUser();
        $this->curl = new OPanda_CurlObject();
    }
    ...
    public function request($method, $uri, $data = array(), $options = array()) {
        ...
        if (empty($options['allow_empty']) && !isset($data)) {
            throw new OPanda_AWeberResponseError($uri);
        }
        return $data;
    }
    ...
    public function getAccessToken() {
        ...
        if (empty($data['oauth_token'])) {
            throw new OPanda_AWeberOAuthDataMissing('oauth_token');
        }
     ...
     * @access public
     * @throws OPanda_AWeberOAuthException
     * @return void
     */
    public function parseAsError($response) {
        if (!empty($response['error'])) {
            throw new OPanda_AWeberOAuthException($response['error']['type'],
    ...
    protected function requiredFromResponse($data, $requiredFields) {
        foreach ($requiredFields as $field) {
            if (empty($data[$field])) {
                throw new OPanda_AWeberOAuthDataMissing($field);
            }
        }
    }
    ...
    public function makeRequest($method, $url, $data=array()) {
        ...
        if (!$resp) {
            ...
            throw new OPanda_AWeberAPIException($error, $url);
        }

        if($resp->headers['Status-Code'] >= 400) {
            $data = json_decode($resp->body, true);
            throw new OPanda_AWeberAPIException($data['error'], $url);
        }
    ...
    private function _sendRequest($handle, $headers = array('Expect:')) {
        ...
        if ($resp) {
            return new OPanda_CurlResponse($resp);
        }
