 ...
 * @version $id$
 */
class OPanda_AWeberServiceProvider implements OPanda_OAuthServiceProvider {
...
class OPanda_AWeberAPIBase {
     ...
     * @access public
     * @return OPanda_AWeberEntry or AWeberCollection
     */
    ...
    protected function readResponse($response, $url) {
        ...
        if (!empty($response['id'])) {
            return new OPanda_AWeberEntry($response, $url, $this->adapter);
        } else if (array_key_exists('entries', $response)) {
            return new OPanda_AWeberCollection($response, $url, $this->adapter);
        }
        return false;
    }
}
 ...
 * @version $id$
 */
class OPanda_AWeberAPI extends OPanda_AWeberAPIBase {
    ...
    public static function getDataFromAweberID($string) {
        list($consumerKey, $consumerSecret, $requestToken, $tokenSecret, $verifier) = OPanda_AWeberAPI::_parseAweberID($string);

        if (!$verifier) {
            return null;
        }
        $aweber = new OPanda_AWeberAPI($consumerKey, $consumerSecret);
    ...
    public function setAdapter($adapter=null) {
        if (empty($adapter)) {
            $serviceProvider = new OPanda_AWeberServiceProvider();
            $adapter = new OPanda_OAuthApplication($serviceProvider);
