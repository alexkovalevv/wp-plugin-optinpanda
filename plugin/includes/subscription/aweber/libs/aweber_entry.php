...
class OPanda_AWeberEntry extends OPanda_AWeberResponse {
    ...
    public function attrs() {
        ...
        if (!empty(OPanda_AWeberAPI::$_collectionMap[$this->type])) {
            foreach (OPanda_AWeberAPI::$_collectionMap[$this->type] as $child) {
    ...
    public function move($list, $last_followup_message_number_sent=NULL) {
        ...
        return new OPanda_AWeberEntry($resource_data, $url, $this->adapter);
    }
     ...
     * @access public
     * @throws OPanda_AWeberResourceNotImplemented
     * @return mixed
     */
    public function __get($value) {
        ...
        if (!empty($this->data) && array_key_exists($value, $this->data)) {
            if (is_array($this->data[$value])) {
                $array = new Opanda_AWeberEntryDataArray($this->data[$value], $value, $this);
        ...
        throw new OPanda_AWeberResourceNotImplemented($this, $value);
    }
     ...
     * @access public
     * @return OPanda_AWeberCollection
     */
    public function findSubscribers($search_data) {
        ...
        return new OPanda_AWeberCollection($data, $url, $this->adapter);
    }
    ...
    public function getParentEntry(){
        ...
        try {
            ...
            return new OPanda_AWeberEntry($data, $url, $this->adapter);
        } catch (Exception $e) {
            return NULL;
        }
    }
    ...
    protected function _parseNamedOperation($data) {
        ...
        foreach($data as $entryData) {
            $results[] = new OPanda_AWeberEntry($entryData, str_replace($this->adapter->app->getBaseUri(), '',
    ...
    protected function _methodFor($entryTypes) {
        ...
        throw new OPanda_AWeberMethodNotImplemented($this);
    }
    ...
    protected function _getCollection($value) {
        if (empty($this->_collections[$value])) {
            $url = "{$this->url}/{$value}";
            $data = $this->adapter->request('GET', $url);
            $this->_collections[$value] = new OPanda_AWeberCollection($data, $url, $this->adapter);
        }
    ...
    protected function _isChildCollection($value) {
        $this->_type();
        if (!empty(OPanda_AWeberAPI::$_collectionMap[$this->type]) &&
            in_array($value, OPanda_AWeberAPI::$_collectionMap[$this->type])) return true;
        return false;
    }
